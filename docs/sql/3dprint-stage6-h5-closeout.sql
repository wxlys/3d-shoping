...
SET `name` = '3D打印业务维护',
    ...
    `content` = '每隔5分钟处理报价过期、待评价超时自动完成和未引用模型清理',
    `type` = 2,
    `month` = 0,
    `week` = 1,
    `day` = 1,
...
SELECT '3D打印业务维护', 'printMaintenance', '每隔5分钟处理报价过期、待评价超时自动完成和未引用模型清理', 2, 0, 1, 1, 1, 5, 0, 0, UNIX_TIMESTAMP() + 300, UNIX_TIMESTAMP(), UNIX_TIMESTAMP(), 0, 1, '', '0 */5 * * * *'
